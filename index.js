...
function cardEffect(event) {
    ...
    const centerY = card.offsetTop + cardWidth/2;
    const positionX = event.clientX - centerX;
    const positionY = event.clientY - centerY;

    const rotateX = ((+1)*25*positionX/(cardHeight/2)).toFixed(2);
    const rotateY = ((-1)*25*positionY/(cardWidth/2)).toFixed(2);
...
function cardTransition() {
    ...
    card.style.transition = 'tranform 400ms';
